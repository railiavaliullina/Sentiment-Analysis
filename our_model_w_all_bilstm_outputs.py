...
# Elmo -> biLSTM -> MaxPool -> 2 layer fc (512)  # (+ dropout)

# Test accuracy : 0.8564814925193787
# Best validation accuracy : 0.8507999777793884
...
def model(x_train, y_train, x_test, y_test, x_valid, y_valid, c_train, c_test, c_valid):
    ...
    tr = tf.placeholder(tf.bool, name='tr')
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')

    # Variables
    global_step = tf.Variable(0, trainable=False, name='global_step')
    weights_fc_1 = tf.get_variable(shape=[channel_dim, channel_dim],
                                   initializer=tf.contrib.layers.xavier_initializer(),
                                   name='weights_fc_1')
    biases_fc_1 = tf.Variable(tf.constant(0.0, shape=[channel_dim]), name='biases_fc_1')
    weights_fc_2 = tf.get_variable(shape=[channel_dim, num_classes],
                                   initializer=tf.contrib.layers.xavier_initializer(),
                                   name='weights_fc_2')
    biases_fc_2 = tf.Variable(tf.constant(0.0, shape=[num_classes]), name='biases_fc_2')
    tf.summary.histogram('weights_fc_1', weights_fc_1)
    tf.summary.histogram('weights_fc_2', weights_fc_2)
    ...
    with tf.name_scope('pooling_layer'):
        max_pooling_output = tf.layers.max_pooling1d(tf.reshape(res, [-1, 4, channel_dim]), pool_size=2, strides=4)
        max_pooling_output = tf.reshape(max_pooling_output, [-1, channel_dim])

    with tf.name_scope('fully_connected_layer_1'):
        fc_layer_outputs_1 = tf.add(tf.matmul(max_pooling_output, weights_fc_1), biases_fc_1)
        batch_norm_1 = tf.layers.batch_normalization(inputs=fc_layer_outputs_1, training=tr)
        fc_layer_outputs_1 = tf.nn.relu(batch_norm_1)

    dropout = tf.nn.dropout(fc_layer_outputs_1, keep_prob)

    with tf.name_scope('fully_connected_layer_2'):
        fc_layer_outputs_2 = tf.add(tf.matmul(dropout, weights_fc_2), biases_fc_2)
        batch_norm_2 = tf.layers.batch_normalization(inputs=fc_layer_outputs_2, training=tr)
        fc_layer_outputs_2 = tf.nn.relu(batch_norm_2)

    with tf.name_scope('softmax_layer'):
        y_ = tf.nn.softmax(fc_layer_outputs_2)
    ...
    sess = tf.Session()
    train_writer = tf.summary.FileWriter('C:/Users/Admin/PycharmProjects/Elmo/tensorboard/our_model_sarcasm_nf', sess.graph)
        ...
        for j in range(num_iter):
            ...
            if j % 100 == 0:
                ...
                for k in range(valid_set_size // batch_size):
                    ...
                    valid_acc = sess.run(accuracy, feed_dict={x: batch_x_valid,
                                                              y: batch_y_valid,
                                                              keep_prob: 1.0,
                                                              tr: False})
                ...
                if np.mean(valid_accuracies) > best_valid_acc:
                    saver.save(sess=sess, save_path='saved_models/our_model_sarcasm_nf/best_model.ckpt')
                    ...
                    with open('saved_models/our_model_sarcasm_nf/info.txt', 'w') as fout:
                        fout.write(text)
    saver.save(sess=sess, save_path='saved_models/our_model_sarcasm_nf/last/last_model.ckpt')
    ...
    for j in range(test_set_size // batch_size):
        ...
        test_acc = sess.run(accuracy, feed_dict={x: batch_x_test, y: batch_y_test, keep_prob: 1.0, tr: False})
