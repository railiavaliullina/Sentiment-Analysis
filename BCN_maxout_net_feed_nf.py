...
def model(x_train, y_train, x_test, y_test, x_valid, y_valid):
    ...
    sess = tf.Session()
    train_writer = tf.summary.FileWriter('fin_models_checkpoints_nf/BCN_w_feedforward_network/tensorboard/train',
                                         sess.graph)
    valid_writer = tf.summary.FileWriter('fin_models_checkpoints_nf/BCN_w_feedforward_network/tensorboard/valid',
                                         sess.graph)
    merged = tf.summary.merge_all()
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
    with open('fin_models_checkpoints_nf/BCN_w_feedforward_network/last/last_epoch.txt', 'r', encoding='utf-8') as f:
        last_ep = f.read()
    if last_ep == '':
        last_ep = 0
    else:
        last_ep = int(last_ep) + 1
    # saver.restore(sess, tf.train.latest_checkpoint('fin_models_checkpoints_nf/BCN_w_feedforward_network/last'))
        ...
        for j in range(num_iter):
            ...
            if j % 100 == 0:
                ...
                if np.mean(valid_accuracies) > best_valid_acc:
                    saver.save(sess=sess, save_path='fin_models_checkpoints_nf/BCN_w_feedforward_network/best/best_model.ckpt')
                    ...
                    with open('fin_models_checkpoints_nf/BCN_w_feedforward_network/best/info.txt', 'w') as fout:
                        fout.write(text)
