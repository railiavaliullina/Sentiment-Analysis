...
class BCN:
    ...
    def create_model(self):
        ...
        with tf.variable_scope("feedforward"):
            ...
            assert dimensions_equal(feedforward_outputs1.shape,
                                    (self.params['batch_size'], self.max_sent_len, self.embed_dim))
            assert dimensions_equal(feedforward_outputs2.shape,
                                    (self.params['batch_size'], self.max_sent_len, self.embed_dim))
        ...
        if self.params['same_bilstm_for_encoder']:
            with tf.variable_scope("bilstm_encoder_scope"):
                ...
                emcoder_outputs1, encoder_outputs2 = tf.split(
                    tf.concat([encoder_raw_outputs[0], encoder_raw_outputs[-1]], 2), 2, axis=0)
        else:
        ...
        assert dimensions_equal(emcoder_outputs1.shape, (
        self.params['batch_size'], self.max_sent_len, self.params['bilstm_encoder_n_hidden'] * 2))
        assert dimensions_equal(encoder_outputs2.shape, (
        self.params['batch_size'], self.max_sent_len, self.params['bilstm_encoder_n_hidden'] * 2))
        ...
        assert dimensions_equal(biattention_outputs1.shape, (
        self.params['batch_size'], self.max_sent_len, self.params['bilstm_encoder_n_hidden'] * 2 * 3))
        assert dimensions_equal(biattention_outputs2.shape, (
        self.params['batch_size'], self.max_sent_len, self.params['bilstm_encoder_n_hidden'] * 2 * 3))
        ...
        assert dimensions_equal(integrate_outputs1.shape, (
        self.params['batch_size'], self.max_sent_len, self.params['bilstm_integrate_n_hidden'] * 2))
        assert dimensions_equal(integrate_outputs2.shape, (
        self.params['batch_size'], self.max_sent_len, self.params['bilstm_integrate_n_hidden'] * 2))

        # Max, mean, min and self-attentive pooling
        with tf.variable_scope("pool"):
            self_pool_weight1 = tf.get_variable("self_pool_weight1",
                                                shape=[self.params['bilstm_integrate_n_hidden'] * 2, 1],
                                                initializer=tf.random_uniform_initializer(-self.W_init, self.W_init))
            self_pool_bias1 = tf.get_variable("self_pool_bias1", shape=[1],
                                              initializer=tf.constant_initializer(self.b_init))
            self_pool_weight2 = tf.get_variable("self_pool_weight2",
                                                shape=[self.params['bilstm_integrate_n_hidden'] * 2, 1],
                                                initializer=tf.random_uniform_initializer(-self.W_init, self.W_init))
            ...
            assert dimensions_equal(pool_outputs1.shape,
                                    (self.params['batch_size'], self.params['bilstm_integrate_n_hidden'] * 2 * 4))
            assert dimensions_equal(pool_outputs2.shape,
                                    (self.params['batch_size'], self.params['bilstm_integrate_n_hidden'] * 2 * 4))
        ...
        with tf.variable_scope("maxout"):
            ...
            assert dimensions_equal(joined.shape,
                                    (self.params['batch_size'], self.params['bilstm_integrate_n_hidden'] * 2 * 4 * 2))
            ...
            assert dimensions_equal(bn1.shape,
                                    (self.params['batch_size'], self.params['bilstm_integrate_n_hidden'] * 2 * 4 * 2))
            maxout_weight1 = tf.get_variable("maxout_weight1",
                                             shape=[self.params['bilstm_integrate_n_hidden'] * 2 * 4 * 2,
    ...
    def train(self, dataset):
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for epoch in range(self.params['n_epochs']):
                print("  ============== Epoch " + str(epoch + 1) + " of " + str(
                ...
                dev_accuracy = self.calculate_accuracy(dataset, sess, inputs1, inputs2, labels, is_training, predict,
                                                       set_name="dev")
                ...
                if epochs_since_last_save >= 7:
                    ...
                    break
            print("Finished training model after " + str(
                best_epoch_number + 1) + " epochs. Model is saved in: " + self.outputdir)
            print("Best dev accuracy: " + str(best_dev_accuracy))
        return best_dev_accuracy

    def calculate_accuracy(self, dataset, sess, inputs1, inputs2, labels, is_training, predict, set_name="test",
                           verbose=False):
    ...
    def test(self, dataset):
        ...
        with tf.Session(graph=graph) as sess:
            ...
            accuracy = self.calculate_accuracy(dataset, sess, inputs1, inputs2, labels, is_training, predict,
                                               verbose=True)
