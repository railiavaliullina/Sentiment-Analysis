...
from elmo_data import UnicodeCharsVocabulary, Batcher
...
def dump_token_embeddings(vocab_file, options_file, weight_file, outfile):     # remove ??
    '''
...
def dump_bilm_embeddings(vocab_file, dataset_file, options_file,
    ...
    vocab = Batcher(vocab_file, max_word_length)  # instead of UnicodeCharsVocabulary class (data__.py)  ??
    batcher = Batcher(vocab_file, max_word_length)
    ...
    with tf.Session(config=config) as sess:
        sess.run(tf.global_variables_initializer())
        sentence_id = 0
        with open(dataset_file, 'r', encoding='utf-8') as fin, h5py.File(outfile, 'w') as fout:
            for line in fin:
