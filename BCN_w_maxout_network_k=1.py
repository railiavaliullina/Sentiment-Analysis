...
def MLP_maxout(input_vector, d, m_, maxout_layer_num):
    weights_maxout = tf.get_variable(shape=[d, m_],
                                     ...
                                     name='weights_maxout{}'.format(str(maxout_layer_num)))
    biases_maxout = tf.Variable(tf.constant(0.1, shape=[m_]),
                                 name='biases_maxout_{}'.format(str(maxout_layer_num)))
    res = tf.add(tf.matmul(input_vector, weights_maxout), biases_maxout)
    return res
...
def model(x_train, y_train, x_test, y_test, x_valid, y_valid):
    ...
    with tf.variable_scope('Pool_X'):
        B_x = tf.add(tf.matmul(tf.reshape(X_y, [-1, 2 * units]), W_at_x), b_at_x)
        B_x = tf.nn.softmax(tf.reshape(B_x, [-1, max_len, 1]))
    ...
    with tf.variable_scope('Pool_Y'):
        B_y = tf.add(tf.matmul(tf.reshape(Y_x, [-1, 2 * units]), W_at_y), b_at_y)
        B_y = tf.nn.softmax(tf.reshape(B_y, [-1, max_len, 1]))
        # weighted summations of each sequence (x_self_pool, y_self_pool)
    ...
    with tf.variable_scope('Maxout_network'):
        ...
        with tf.variable_scope('Maxout_layer_1'):
            ...
            maxout_layer_1 = MLP_maxout(batch_norm_1, d_1, d_1, maxout_layer_num=1)
            maxout_layer_1 = max_out(maxout_layer_1, d_2)

        with tf.variable_scope('Maxout_layer_2'):
            dropout_2 = tf.nn.dropout(tf.reshape(maxout_layer_1, [-1, d_2]), keep_prob)
            batch_norm_2 = tf.layers.batch_normalization(inputs=dropout_2, training=training)
            maxout_layer_2 = MLP_maxout(batch_norm_2, d_2, d_2, maxout_layer_num=2)
            maxout_layer_2 = max_out(maxout_layer_2, d_3)

        with tf.variable_scope('Maxout_layer_3'):
            dropout_3 = tf.nn.dropout(tf.reshape(maxout_layer_2, [-1, d_3]), keep_prob)
            batch_norm_3 = tf.layers.batch_normalization(inputs=dropout_3, training=training)
            maxout_layer_3 = MLP_maxout(tf.reshape(batch_norm_3, [-1, d_3]), d_3, 2, maxout_layer_num=3)
            maxout_layer_3 = tf.reshape(max_out(maxout_layer_3, 1), [-1, num_classes])
    ...
    sess = tf.Session()
    train_writer = tf.summary.FileWriter('C:/Users/Admin/PycharmProjects/Elmo/tensorboard/BCN/maxout/changed', sess.graph)
