...
def model(x_train, y_train, x_test, y_test, x_valid, y_valid, c_train, c_test, c_valid):
    ...
    weights_fc = tf.get_variable(shape=[2*units, num_classes], initializer=tf.contrib.layers.xavier_initializer(), name='weights_fc')
    ...
    outputs, _ = tf.nn.bidirectional_dynamic_rnn(forward_lstm, backward_lstm, dropout, dtype=tf.float32, sequence_length=cur_sent_len)
        ...
        for j in range(num_iter):
            ...
            if j % 100 == 0:
                ...
                for k in range(valid_set_size // batch_size):
                    ...
                    valid_acc = sess.run(accuracy, feed_dict={x: batch_x_valid, y: batch_y_valid, keep_prob: 1.0, training: False})
                ...
                if np.mean(valid_accuracies) > best_valid_acc:
                    saver.save(sess=sess, save_path='saved_models/LSTM(best)_model_not_fixed/best_model.ckpt')
                    ...
                    with open('saved_models/LSTM(best)_model_not_fixed/info.txt', 'w') as fout:
                        fout.write(text)
