...
# Elmo -> Conv1D -> MaxPooling1D -> biLSTM -> fc
...
def model(x_train, y_train, x_test, y_test, x_valid, y_valid, c_train, c_test, c_valid):
    ...
    weights_conv = tf.get_variable(shape=[3, vec_dim, vec_dim],
                                   initializer=tf.contrib.layers.xavier_initializer(),
                                   name='weights_conv1d')
    conv_biases = tf.Variable(tf.constant(0.1, shape=[vec_dim]), name='biases_conv1d')
    ...
    tf.summary.histogram('weights_conv1d', weights_conv)
    tf.summary.histogram('weights_fc', weights_fc)

    # Model
    with tf.name_scope('Conv1d_layer'):
        conv1d_layer = tf.add(tf.nn.conv1d(x, weights_conv, stride=2, padding='SAME'), conv_biases)

    with tf.name_scope('Pooling_layer'):
        max_pooling_output = tf.layers.max_pooling1d(conv1d_layer, pool_size=2, strides=2)
    ...
    with tf.name_scope('bilstm_layer'):
        ...
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(forward_lstm, backward_lstm, max_pooling_output, dtype=tf.float32,
    ...
    with tf.name_scope('fully_connected_layer'):
        fc_layer_outputs = tf.add(tf.matmul(res, weights_fc), biases_fc)
        batch_norm = tf.layers.batch_normalization(inputs=fc_layer_outputs, training=tr)
        fc_layer_outputs = tf.nn.relu(batch_norm)
    ...
    sess = tf.Session()
    train_writer = tf.summary.FileWriter('C:/Users/Admin/PycharmProjects/Elmo/tensorboard/our_model_conv_nf', sess.graph)
        ...
        for j in range(num_iter):
            ...
            if j % 100 == 0:
                ...
                if np.mean(valid_accuracies) > best_valid_acc:
                    saver.save(sess=sess, save_path='saved_models/our_model_conv_nf/best_model.ckpt')
                    ...
                    with open('saved_models/our_model_conv_nf/info.txt', 'w') as fout:
                        fout.write(text)
    saver.save(sess=sess, save_path='saved_models/our_model_conv_nf/last/last_model.ckpt')
    ...
    for j in range(test_set_size // batch_size):
        ...
        test_acc = sess.run(accuracy, feed_dict={x: batch_x_test, y: batch_y_test, tr: False})
